            ...
            jQuery('#sb_instagram.sbi').each(function(){ //Ends on line 1676
                ...
                function sbiCreateFeed(var_this,feedOptions) {
                    ...
                    if(getType == 'hashtag'){
                        ...
                        jQuery.each(feedOptions.feedsArray,function(index,value) {
                            var splits = value.split('.'),
                                ...
                                feedOptions.feedsArrayFinal.push(trimmed);
                                var moreSplits = trimmed.split('.');
                            feedOptions.idsArrayFinal.push(moreSplits[0]);
                        });
                    ...
                    jQuery.each( looparray, function( index, entry ) {
                        ...
                        if(getType == 'user'){
                            apiCall = "https://api.instagram.com/v1/users/"+ feedOptions.idsArrayFinal[index] +"/media/recent?access_token=" + feedOptions.feedsArrayFinal[index]+"&count=33";
                        } else if(getType == 'coordinates') {
                            ...
                            function sbiSetPhotoHeight(){
                                ...
                                if( imgRes !== 'thumbnail' ){
                                    ...
                                    if( !$self.hasClass('sbi_disable_mobile') ){
                                        ...
                                        if( sbiWindowWidth <= 480 && parseInt(cols) > 2 ) sbi_num_cols = 2;
                                    }
                                    ...
                                    success: function(data) {
                                        ...
                                        if(typeof sbiErrorResponse !== 'undefined'){
                                            ...
                                            if( sbiErrorResponse.indexOf('access_token') > -1 ){
                                                sbiErrorMsg += 'Access Token is not valid or has expired';
                                                sbiErrorDir = "There's an issue with the Instagram Access Token that you are using. Please obtain a new Access Token on the plugin's Settings page. If you continue to have an issue with your Access Token then please see this FAQ for more information https://smashballoon.com/my-instagram-access-token-keep-expiring/. ";
                                            } else if( sbiErrorResponse.indexOf('user does not exist') > -1 || sbiErrorResponse.indexOf('you cannot view this resource') > -1 ){
                                                sbiErrorMsg += 'User IDs does not exist, is invalid, or private';
                                                sbiErrorDir = "Please double check the Instagram User ID that you are using and ensure that it is valid and not from a private account. To find your User ID simply enter your Instagram user name into this tool https://smashballoon.com/instagram-feed/find-instagram-user-id/";
                                            } else if( sbiErrorResponse.indexOf('invalid media id') > -1 ){
                                                sbiErrorMsg += 'Post Id does not exist or is invalid.';
                                                sbiErrorDir = "Please double check the media (post) id is correct.";
                                            }

                                            //Add the error message to the feed
                                            console.log('Instagram Feed Error:', sbiErrorMsg);
                                            console.log('Instagram Feed Directions:', sbiErrorDir);
